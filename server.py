...
<body>
    <div class="container">
        ...
        <div class="card">
            <div class="stats">
                ...
                <div class="stat-box" style="color:#ff4444;"><span class="stat-num" style="color:#ff4444;">{{ rendimiento.retrasos }}</span><span class="stat-label">Retrasos</span></div>
            </div>
        </div>
        <div class="card">
    ...
    <script>
        // Este script revisa si hubo cambios en el servidor cada 3 segundos
        let ultimaCarga = "{{ historial|length }}";
        let ultimoEstado = "{{ historial[-1].estado if historial else '' }}";

        setInterval(async () => {
            const res = await fetch('/verificar_cambios');
            const data = await res.json();
            
            // Si hay una tarea nueva o si el estado de la última tarea cambió, refresca
            if (data.total_historial != ultimaCarga || data.ultimo_estado != ultimoEstado) {
...
def verificar_cambios():
    est = db['historial'][-1]['estado'] if db['historial'] else ""
    return jsonify({"total_historial": len(db['historial']), "ultimo_estado": est})
    ...
    for t in db['historial']:
        if t['id'] == data.get('id'):
            if t['estado'] == "PENDIENTE":
                t['estado'] = data.get('estado')
                if "RETRASO" in t['estado']: db['rendimiento']['retrasos'] += 1
