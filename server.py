...
# ── RUTA AJAX: REGISTRAR OPERADOR SIN RECARGAR ──
  ...
  .then(data => {
    if(data.success) {
      ...
      newRow.innerHTML = `
        ...
        <div class="op-avatar ${currentAv}">${data.iniciales}</div>
        <div>
        ...
        <div>
          <div onclick="eliminarOperadorAjax('${data.nombre}')" class="del-btn"><i class="ti ti-trash"></i></div>
        </div>
      `;
...
function eliminarOperadorAjax(nombre) {
  ...
  .then(data => {
    if(data.success) {
      ...
      const opt = document.querySelector(`#dest-select option[value="${nombre}"]`);
      if(opt) opt.remove();
    } else {
      document.getElementById('console-msg').textContent = `ERROR: ${data.error}`;
    }
